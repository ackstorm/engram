...
export interface ContradictionResult {
  ...
  explanation: string;
  /** When the old fact stopped being true, if inferable (ISO 8601) */
  oldFactEndDate?: string;
}
...
export function findContradictionCandidates(
  ...
  return existingMemories.filter(existing => {
    ...
    // Boost: if both contain numbers/percentages about the same entity,
    // they're strong contradiction candidates even with low topic overlap
    const numberPattern = /\b\d+(\.\d+)?%?(?:\s*(points?|pts?|tokens?|memories|questions?|convs?|conversations?))?/g;
    const newHasNumbers = numberPattern.test(newMemory.content);
    numberPattern.lastIndex = 0; // Reset regex state
    const existingHasNumbers = numberPattern.test(existing.content);
    if (newHasNumbers && existingHasNumbers && sharedEntities.length >= 1) {
      return true; // Both mention numbers about the same entity — likely fact update
    }

    return hasTopicOverlap || sharedEntities.length >= 2;
  });
}
