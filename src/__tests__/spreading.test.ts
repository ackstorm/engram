...
describe('Spreading Activation', () => {
  ...
  it('edge type weights affect propagation strength', async () => {
    ...
    // Both should be reachable via spreading activation from the root
    // (exact ranking may vary based on other scoring factors like recency)
    expect(supportIdx).not.toBe(-1);
    expect(temporalIdx).not.toBe(-1);
  });
});
